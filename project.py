...
question = 'якого кольру негри'
wrong1 = "білі"
wrong2 = "жовті"
wrong3 = "червоні"
correct = "чорні"
List = [rbtn_1,rbtn_2,rbtn_3,rbtn_4]
shuffle(List)
answer = List[0]
w1 = List[1]
w2 = List[2]
...
def chek_result():
    AnswC = answer.isChecked()
    if AnswC:
        lb_Result.setText('ПРАВИЛЬНО')
        RadioGroupBox.hide()
        AnsGroupBox.show()
    else:
        AnswU = w1.isChecked() or w2.isChecked() or w3.isChecked()
        if AnswU:
                lb_Result.setText('НЕПРАВИЛЬНО')
                RadioGroupBox.hide()
                AnsGroupBox.show()
    btn_OK.setText('Продовжити')
    if btn_OK == 'Продовжити':
...
def sleep_button():
    window.hide()
    time.sleep(box_Minutes.value())
    window.show()

    

show_data()
btn_OK.clicked.connect(chek_result)
btn_Sleep.clicked.connect(sleep_button)
